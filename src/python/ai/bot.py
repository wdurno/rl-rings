...
class Agent(object):
    ...
    def learn(self):
        ...
        transitions = self.memory.sample(self.batch_size) ## TODO replace with Horovod-sampled dataset 
    ...
    def apply_grads(self, grads): 
        ...
        for p, g in zip(self.policy.parameters(), grads): ## TODO replace with .step()  
            p.grad = g 
...
def state_to(pov: list, compass: list):
    ...
    ## to cpu 
    pov = pov.to(torch.device(device)) 
    compass = compass.to(torch.device(device)) 
    ## format 
    state = {
            'pov': pov, 
            'compass': compass
